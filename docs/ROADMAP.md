...
**2026-06-29 — recruited prospect signup: profile prefill + Google auto-connect.** Closed the conversion leak where a claimed prospect hit a fully blank onboarding even though we already had their data. At provision time (`provisionContractor` → new `attributeAndPrefillFromProspect`, so it covers both email/password and OAuth signups) we resolve the signup back to a scraped prospect via `resolveProspectForSignup` (`convert.ts`): **cookie → exact business-email → unique business-domain match**, free mailbox domains (gmail/yahoo/…) excluded — so a prospect who signs up on their own matches too, not just claim-link clicks. We then mark the prospect converted and **prefill** the profile from scraped data: `contractors.companyName`, `users.phone` (E.164), and a default coverage circle from the prospect's lat/lng (radius 40km). Onboarding already reads those tables, so the wizard shows them with **no form change**. Async, a `contractor/claimed` Inngest fn (`contractor-claimed.ts`) **auto-connects their Google listing** from the stored place_id (`contractor_prospects.source_ref`): NEW server-side Place Details fetcher (`google-place-details.ts`, reuses `GOOGLE_PLACES_API_KEY` — needs the Places **"atmosphere"/Place Details** billing for reviews+photos; photos resolved key-free via `skipHttpRedirect`) imports reviews → `external_reviews` and photos → `external_media` (display-only; does NOT touch the ranking columns `avg_rating`/`total_reviews`). The reusable connect core was extracted from the `'use server'` action into `google-connect.ts` (`applyContent`/`upsertGoogleConnection`/`importGooglePlace`) so the job and the manual integrations UI share it; idempotent on the `(contractor, google, place_id)` unique index. No schema change.

**2026-06-29 — recruitment outreach: follow-ups + human-sounding email.** Cold prospects are now re-nudged as fresh demand lands in their area instead of getting one email ever. `recruitment-discover` runs `sendAreaOutreach` (geo-radius `ST_DWithin`, `OUTREACH_AREA_RADIUS_METERS` = discovery radius) on every `lead/awaiting-coverage` event — even when the Places scan is debounced — so each new job in an uncovered area sends the next touch. Bounded by a lifetime cap `MAX_OUTREACH_EMAILS` = 4 and stopped the instant the prospect signs up (`converted_to_contractor_id`); `OUTREACH_FOLLOWUP_COOLDOWN_HOURS` = 0 (every new job nudges immediately — raise to throttle). New `contractor_prospects.outreach_count` counter (migration `0030`); `outreach-sync.ts` shares one eligibility gate between the global admin batch and the area sender. The email (`recruitment-email.ts`) was rewritten to read like a founder typed it: **first-touch vs follow-up** variants, merges the prospect's company name / city / Google rating, varied subject + phrasing per prospect (stable hash → less templated footprint), a plaintext part, short, no em-dashes. Signed by `RECRUITMENT_SIGNER_NAME` (NEW optional env — falls back to `RECRUITMENT_FROM_NAME`). Note: `docs/launch-campaign.md` remains the **manual** campaign copy (founder-sent Facebook/DM/bulk); this entry covers the **automated** per-prospect engine email.

**2026-06-28 — removed the /coming-soon out-of-area geo-gate.** The homeowner lead funnel is no longer country-gated — anyone can post a job (`/get-a-quote` is open). Deleted: the `/coming-soon` page + its `ComingSoon` component, the proxy geo-gate (`GEO_GATED_PATHS` + redirect + `AREA_BYPASS` cookie), the `continueAsLocal` bypass action, the `/api/geo-debug` route, and the gate unit test. Trimmed `service-areas.ts` (removed `OPERATING_COUNTRIES`/`OPERATING_AREAS_LABEL`/`isServedCountry`/`regionLabel`/`AREA_BYPASS_*`; kept the SEO state config + GEO headers). KEPT: the `waitlist` table + `joinWaitlist` + the public `AnnouncementBar` (early-access "join waitlist" bar) — it's a separate pre-launch element, not the coming-soon page. The reusable dashboard `ComingSoon` placeholder (storm-alerts/disputes/integrations "feature coming soon") is unrelated and untouched.

**2026-06-28 — recruitment engine, Phases 1 + 2 (discovery → enrich → outreach → claim).** Built the supply side on top of Phase 0. **Phase 1 (in-app):** server-side Google Places Text Search (`src/lib/integrations/google-places-server.ts`, NEW `GOOGLE_PLACES_API_KEY`) discovers roofers near an uncovered point; `recruitment-discover` Inngest fn (triggered by `lead/awaiting-coverage` debounced per city, and by admin `startCampaign`) upserts them into `contractor_prospects` and queues website-but-no-email ones into `prospect_enrichment_jobs` (migration `0029`). Verified prospects are emailed via **Resend on a separate domain** (`recruitment-email.ts` — free at low volume, isolates the transactional `hommy.online` reputation; chosen over paid tools like Instantly/Smartlead). `outreach-sync.ts` sends with hard gates (verified email, confidence ≥ threshold, not opted-out), CAN-SPAM footer + one-click List-Unsubscribe; bounces/complaints sync back at `/api/webhooks/resend` and `/unsubscribe/<token>` → `email_opt_outs` suppression + prospect status. A signed `/claim/<token>` route records the click, drops an attribution cookie, and routes to contractor signup; `provisionContractor` links the conversion. Admin `/admin/recruitment` gained per-area "Find roofers" + "Send to verified prospects" buttons. **Phase 2 (separate Python service, scaffolded in `worker/recruitment/`):** drains `prospect_enrichment_jobs` with `FOR UPDATE SKIP LOCKED`, runs the email waterfall (ScrapeGraphAI crawl → Hunter domain-search → Hunter verify), writes `email`+`email_confidence` or marks `no_email`. New env: `GOOGLE_PLACES_API_KEY`, `UNSUBSCRIBE_SECRET`, `RECRUITMENT_FROM_EMAIL`/`RECRUITMENT_FROM_NAME`/`RECRUITMENT_REPLY_TO`/`RECRUITMENT_MAILING_ADDRESS` (separate Resend domain), `RESEND_WEBHOOK_SECRET`, `HUNTER_API_KEY`, `SCRAPEGRAPH_API_KEY` (worker). Reuses existing `RESEND_API_KEY`. The full loop closes: a recruited roofer signs up → verifies → Phase 0's `contractor/eligible` auto-matches the waiting lead. No-email companies are skipped (no SMS).

**2026-06-28 — recruitment engine, Phase 0 (no-match → awaiting coverage → auto-match).** When a homeowner posts and zero verified contractors cover the area, the lead is flagged `leads.awaiting_coverage` (boolean, migration `0028`; NOT a new status value — avoids auditing every `status='open'` site) instead of silently dead-ending. The homeowner gets an honest message (wizard toast + the previously-fake dashboard/sheet copy now backed by the real flag), and the SLA cron no longer auto-expires these. The instant a covering contractor becomes eligible — verified (`decideVerification`) or a verified company expands coverage (`addServiceArea`) — a `contractor/eligible` Inngest event runs the new `contractor-eligible` fn: inverted `ST_Covers` finds awaiting leads it now covers, inserts `lead_recipients` (idempotent), clears the flag, notifies the homeowner (`coverage_matched:*`) and the contractor (reuses `LEAD_CREATED`). Admin gains `/admin/recruitment` (uncovered demand by city/state). Also added empty `contractor_prospects` + `email_opt_outs` tables for Phase 1. Full design in the plan; Phase 1 (Places discovery + cold outreach via Instantly/Smartlead + claim flow) and Phase 2 (Python ScrapeGraphAI/Hunter enrichment worker) follow. See `docs/launch-campaign.md` for the outreach copy.
