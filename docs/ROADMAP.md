...
**2026-06-28 — recruitment engine, Phases 1 + 2 (discovery → enrich → outreach → claim).** Built the supply side on top of Phase 0. **Phase 1 (in-app):** server-side Google Places Text Search (`src/lib/integrations/google-places-server.ts`, NEW `GOOGLE_PLACES_API_KEY`) discovers roofers near an uncovered point; `recruitment-discover` Inngest fn (triggered by `lead/awaiting-coverage` debounced per city, and by admin `startCampaign`) upserts them into `contractor_prospects` and queues website-but-no-email ones into `prospect_enrichment_jobs` (migration `0029`). Verified prospects are exported to a dedicated cold-email tool (Instantly/Smartlead) via a swappable adapter (`outreach-adapter.ts` + `outreach-sync.ts`) with hard gates (verified email, confidence ≥ threshold, not opted-out); status syncs back at `/api/webhooks/outreach` → `outreach_status` + `email_opt_outs` suppression. A signed `/claim/<token>` route records the click, drops an attribution cookie, and routes to contractor signup; `provisionContractor` links the conversion. Admin `/admin/recruitment` gained per-area "Find roofers" + "Export verified" buttons. **Phase 2 (separate Python service, scaffolded in `worker/recruitment/`):** drains `prospect_enrichment_jobs` with `FOR UPDATE SKIP LOCKED`, runs the email waterfall (ScrapeGraphAI crawl → Hunter domain-search → Hunter verify), writes `email`+`email_confidence` or marks `no_email`. New env: `GOOGLE_PLACES_API_KEY`, `UNSUBSCRIBE_SECRET`, `OUTREACH_TOOL_API_KEY`/`OUTREACH_CAMPAIGN_ID`/`OUTREACH_WEBHOOK_SECRET`, `HUNTER_API_KEY`, `SCRAPEGRAPH_API_KEY` (worker). The full loop closes: a recruited roofer signs up → verifies → Phase 0's `contractor/eligible` auto-matches the waiting lead. No-email companies are skipped (no SMS).
