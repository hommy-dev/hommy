...
export type LoadingProps = DotMatrixCommonProps;
...
export function Loading({
  ...
  opacityMid,
  opacityPeak,
  // `fill`: size the loader from its own className box (px, rem, vw, size-*),
  // scaling the fixed-pixel dot matrix to fit. Use this for vw-based sizing —
  // the numeric `size`/`dotSize` props can't express viewport units.
  fill = false
}: LoadingProps & { fill?: boolean }) {
  ...
  // `fill` mode: measure the className-sized box and scale the px matrix to fit,
  // so any CSS unit (incl. vw) controls the rendered size.
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      if (r.width > 0 && r.height > 0 && matrixWidth > 0 && matrixHeight > 0) {
        setFitScale(Math.min(r.width / matrixWidth, r.height / matrixHeight));
      }
    };
    measure();
  ...
  const matrix = (
    <div
      ...
      className={cx("dmx-root", `dmx-dot-shape-${dotShape}`, muted && "dmx-muted", dmxBloomRootActive(bloom, halo) && "dmx-bloom", !useWrapper && !fill && className)}
      style={matrixStyle}
